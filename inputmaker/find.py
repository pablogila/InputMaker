...
def line(keyword:str, file:str, lines_below:int=0, stop_at:int=0):
    '''
    ...
    Default is `lines_below=0`, only returns the target line.\n
    `stop_at` specifies the number of matches to be found before stopping.
    Defaults to 0 to return all possible matches. Set it to 1 to return only one match,
    or to negative numbers to start the search from the enf of the file upwards.

    TO-IMPLEMENT: lines_below working
    '''
    file_path = get(file)  # Assuming `get` function retrieves the correct file path
    matches = []
